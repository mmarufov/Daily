...
|---|---|---|
| 0. Freeze the ruler | `docs/stages/s5-*`, `backend/tests/test_preferences.py`, new `backend/evals/reader/` fixtures | Inventory every writer/consumer; independent expected profiles/policies; migration precedence and limits recorded; baseline and scope approved. No runtime feature yet. |
...
| 7. Integrated verification | new hosted S5 verification script, `.github/workflows/backend-tests.yml`, backend suites, injected Swift/unit/UI tests; `docs/stages/s5-implementation-status.md` | Full baseline regressions, real hosted transactional proof, migration/restart/rollback, release client build, approved quality/spend/latency report. Report unavailable evidence as pending. |
