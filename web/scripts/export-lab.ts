...
/**
 * THE EXPORT READS NO GIT.
 *
 * It used to, and that is what made the staleness gate fail in CI and nowhere
 * else. The gate compares regenerated bytes against committed bytes, so every
 * input has to be a committed byte. `git log --format=%h` is not: `%h` honours
 * `core.abbrev`, which defaults to `auto` and is derived from the repository's
 * object count — so the same commit abbreviates to 7 characters in one clone
 * and 8 in another. The embedded patch made it worse by carrying git's
 * `index <blob>..<blob>` line, abbreviated the same way, into all seven runs
 * at once. That is why every file went stale together.
 *
 * Provenance that is genuinely a fact about a *run* is now recorded by the
 * orchestrator when the run happens and read back out of the event log.
 * Provenance about the *inputs* is a content hash over those inputs. Both are
 * committed bytes, so this export is a pure function of the tree.
 */
function hashFiles(paths: readonly string[]): string {
  ...
  const walk = (rel: string): void => {
    ...
    if (statSync(abs).isDirectory()) {
      for (const entry of readdirSync(abs).sort()) walk(`${rel}/${entry}`)
      return
    }
    parts.push(`${rel}:${sha256(readFileSync(abs, 'utf8'))}`)
  }
  for (const p of [...paths].sort()) walk(p)
  return sha256(parts.join('\n')).slice(0, 16)
}

/** Everything a run is derived from: cases, records, candidates, event logs. */
const EVIDENCE_PATHS = [
  'backend/lab/contract',
  'backend/lab/cases',
  'backend/lab/records',
  'backend/lab/runs',
]
...
const INPUTS_SHA256 = hashFiles(EVIDENCE_PATHS)
const EVALUATOR_SHA256 = hashFiles(EVALUATOR_PATHS)
