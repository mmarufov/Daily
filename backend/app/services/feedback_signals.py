...
WEIGHT_FLOOR, WEIGHT_CEILING = _reward.WEIGHT_FLOOR, _reward.WEIGHT_CEILING
ADJUSTMENT_FLOOR, ADJUSTMENT_CEILING = _reward.ADJUSTMENT_FLOOR, _reward.ADJUSTMENT_CEILING
HALF_LIFE_DAYS = _reward.HALF_LIFE_DAYS
...
def apply_feedback(conn, user_id: str, article_id: str, action: str) -> int:
    ...
    if action not in FEEDBACK_DELTAS:
        return 0
    # S10 C: route through the one shared reward function rather than a local
    # dict lookup. Currently equivalent to FEEDBACK_DELTAS[action] (no implicit
    # terms apply here -- explicit-action feedback has none to add), but this
    # is now the single call site any future implicit term also flows through.
    delta = _reward.reward(action)
    ...
    try:
        ...
        with conn.cursor() as cur:
            for kind, value in pairs:
                # S10 A4: decay the STORED weight to now() before adding the new
                # delta, in the same statement. Previously decay only applied at
                # read time (load_feedback_signals), so a year-old -1.0 never
                # relaxed before a fresh +0.2 landed on top of it -- a small
                # positive correction could not undo an old, faded negative one.
                # Matches the fixed pattern in reader_feedback.py's S5 UPSERT.
                cur.execute(
                    """
                    ...
                    ON CONFLICT (user_id, kind, value) DO UPDATE SET
                        weight = GREATEST(%s, LEAST(%s,
                            public.user_feedback_signals.weight * power(0.5,
                                GREATEST(0, extract(epoch FROM (now() - public.user_feedback_signals.updated_at)))
                                / (%s * 86400))
                            + EXCLUDED.weight)),
                    ...
                    (user_id, kind, value, delta * KIND_FACTORS.get(kind, 0.5),
                     WEIGHT_FLOOR, WEIGHT_CEILING, HALF_LIFE_DAYS),
                )
        return len(pairs)
    except Exception:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                ...
                WHERE user_id = %s
                  AND event_type IN ('not_relevant', 'less_like_this', 'already_knew', 'hide_source')
                """,
                (user_id,),
            )
