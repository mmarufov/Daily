...
async def get_personalized_feed(
    ...
    if not force_refresh:
        ...
        if cached is not None:
            ...
            # If cache exists but nothing is relevant, it's likely pre-migration
            # data where the column defaulted to false — treat as stale
            if relevant:
                return relevant[:limit]
            logger.info("Cache for user %s has no relevant articles; treating as stale", user_id)
